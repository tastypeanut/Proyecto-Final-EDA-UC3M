...
public class Student {
	...
	public String toString() {
		return "Student [email=" + email + ", city=" + city + ", campus=" + campus + ", blocks=" + blocks
				+ ", date_sign_in=" + date_sign_in + "]";
	}

	/**
	 * method that checks if the invoking object (this) is equals to the input
	 * object obj
	 * 
	 * @param obj
	 * @return
	...
	public boolean equals(Student obj) {
		...
		Student other = (Student) obj; // Para q sirve esto, y porque crea un objeto así
		...
		if (date_sign_in == null) {
			if (other.date_sign_in != null)
				return false;
		} else if (!date_sign_in.equals(other.date_sign_in))
			return false;
	...
	public static final String[] EMAILS = { "isa@uc3m.es", "pmf@uc3m.es", "har@uc3m.es", "lou@uc3m.es", "cde@uc3m.es",
			"jlm@uc3m.es", "brb@uc3m.es", "igr@uc3m.es", "jpa@uc3m.es", "jlm@uc3m.es", "ccr@uc3m.es", "rva@uc3m.es",
			"aab@uc3m.3s", "bgg@uc3m.es", "dcr@uc3m.es" };

	public static final String[] CITIES = { "GETAFE", "LEGANES", "COLMENAREJO", "FUENLABRADA", "ALCORCON", "BRUNETE",
			"MADRID" };

	/**
	 * Returns a random date between 2010-1-1 and 2019-1-1. ISO-8601 calendar
	...
	public static LocalDate randDate() {
		...
		long randomDay = minDay + random.nextInt(maxDay - minDay); // No lo pillamos
	...
	public static Student createRandomStudent() {
		...
		int index_campus = new Random().nextInt(Campus.values().length); // Que concho!!!
